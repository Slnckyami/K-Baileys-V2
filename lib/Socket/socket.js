...
export const makeSocket = (config) => {
    ...
    if (syncDisabled) {
        logger.warn('⚠️ DANGER: DISABLING ALL SYNC BY KYAMI SILENCE PREVENTS BAILEYS FROM ACCESSING INITIAL LID MAPPINGS, LEADING TO INSTABILIY AND SESSION ERRORS');
    }
    ...
    /**
     * Returns true only once the WS transport is open AND the noise handshake /
     * WA pairing for this connection has actually completed (i.e. we have a
     * logged-in account). Sending stanzas before this is true produces
     * malformed/out-of-sequence traffic that WA's server treats as anomalous.
     */
    const isSocketReady = () => !!ws.isOpen && !!authState.creds?.me?.id;
    return {
        type: 'md',
        ws,
        ev,
