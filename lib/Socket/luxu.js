...
export default class imup {
    ...
    detectType(content) {
        ...
        if (content.album) return 'ALBUM_V2'    // levvleys-style: { album: [...], caption }
        if (content.albumMessage) return 'ALBUM' // kyami legacy: { albumMessage: [...] }
    ...
    /**
     * Levvleys-style album send.
     * Usage: sock.sendMessage(jid, { album: [{ image }, { video }], caption: 'text' })
     * Caption diauto assign ke item pertama kalau belum ada caption-nya sendiri.
     */
    async handleAlbumV2(content, jid, userJid, options = {}) {
        const { album, caption } = content
        if (!Array.isArray(album) || album.length === 0) {
            throw new Error('album harus array dan tidak boleh kosong')
        }
        // caption top-level → otomatis masuk item pertama kalau belum ada
        if (caption && !album[0].caption) {
            album[0].caption = caption
        }
        let mediaHandle
        // Kirim container albumMessage dulu
        const albumMsg = this.utils.generateWAMessageFromContent(jid, {
            albumMessage: {
                expectedImageCount: album.filter(m => 'image' in m).length,
                expectedVideoCount: album.filter(m => 'video' in m).length,
            }
        }, { userJid, ...options })
        await this.relayMessage(jid, albumMsg.message, {
            messageId: albumMsg.key.id
        })
        // Loop tiap media, generate + relay satu-satu
        for (const media of album) {
            ...
            const mediaMsg = await this.utils.generateWAMessage(jid, msgContent, {
                userJid,
                upload: async (readStream, opts) => {
                    const up = await this.waUploadToServer(readStream, {
                        ...opts,
                        newsletter: jid.endsWith('@newsletter')
                    })
            ...
            await this.relayMessage(jid, mediaMsg.message, {
                ...
                ...(mediaHandle ? { additionalAttributes: { media_id: mediaHandle } } : {})
            })
        }
        return albumMsg
    }
                    ...
                    eventMessage: {
                        contextInfo: {
                            ...
                            forwardedNewsletterMessageInfo: {
                                ...
                                newsletterJid: "120363432116778390@newsletter",
                                serverMessageId: 1
                            }
                        },
            ...
            pollResultSnapshotMessage: {
                ...
                contextInfo: {
                    ...
                    forwardedNewsletterMessageInfo: {
                        ...
                        newsletterJid: pollData.newsletter?.newsletterJid || "120363432116778390@newsletter",
    ...
    async handleOrderMessage(content, jid, quoted) {
        ...
        const Haha = await this.utils.generateWAMessageFromContent(jid, {
            orderMessage: {
                orderId: orderData.orderId || ("KYAMI" + Date.now()),
                ...
                token: orderData.token || "KYAMI_EXAMPLE_TOKEN",
